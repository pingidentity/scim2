 ...
 * individual write request that is included within a {@link BulkRequest}. For
 * each bulk operation sent, a SCIM service will reply with a
 * {@link BulkOperationResult}, which represents the response/summary of the
 * server-side processing. For more background on SCIM bulk requests and
 * responses, see {@link BulkRequest}.
 * <br><br>
 *
 ...
 *        in a POST operation, this represents the user that should be created.
 * </ul>
 * <br><br>
 *
 ...
 *   BulkOperation op = BulkOperation.put("/Users/fa1afe1", user);
 * </code></pre>
 * <br><br>
 *
 * Because bulk operations can contain data of many forms, this class contains
 * the following helper methods to help easily extract the {@code data} field
 * into useful forms:
 * <ul>
 *   <li> {@link #getDataAsScimResource()}: Reformats the JSON object node into
 *                                          a ScimResource-based object.
 *   <li> {@link #getPatchOperationList}: Reformats the JSON object node into a
 *                                        list of {@link PatchOperation} fields.
 * </ul>
 *
 * <h2>Bulk IDs</h2>
 * Bulk IDs are useful to reference resources that will be created by a bulk
 * request, but don't yet exist. For example, consider a bulk request that
 * creates one user, as well as one group that contains the user. To accomplish
 * this in a single request, this can be represented with:
 * <pre>
 * {
 ...
 * member, which is done by referencing its bulk ID.
 *
 * @see BulkRequest
 */
...
public abstract class BulkOperation
{
  ...
  /**
   * Sets the bulk ID of the bulk operation. Since bulk IDs may only be
   * used for POST requests, this method does not perform any action if this
   * call does not target a {@code POST}.
   *
   ...
   * @return        This object.
   *
   * @throws BulkRequestException   If a caller attempts setting a bulk
   ...
   * <br><br>
   *
   * This method will not take any action for bulk post operations. When a new
   * resource is contained in a bulk request, that resource does not exist until
   * the bulk request has been processed successfully. Thus, a bulk post
   * operation cannot reference any resource with a version tag, since the SCIM
   * service has not yet assigned a version tag value.
   *
   ...
   * <br><br>
   *
   * This method returns the data as a JSON ObjectNode. If a usable Java object
   * is required, use {@link #getDataAsScimResource()} instead.
   *
  ...
  /**
   * Fetches the {@code data} of this bulk operation as a {@link ScimResource}
   * based POJO. By default, if a suitable class cannot be found, a
   * {@link com.unboundid.scim2.common.GenericScimResource GenericScimResource}
   * will be returned.
   * <br><br>
   *
   ...
   * the {@link BulkResourceMapper}. However, the data field may be manually
   * converted to any class with {@link JsonUtils#nodeToValue}:
   * <pre><code>
   ...
   *          {@code null} if the data field is null (i.e., in delete requests).
   *
   ...
   * confusion with the individual {@link PatchOperation} objects contained
  ...
  protected static final class BulkPatchOperation extends BulkOperation
  {
        ...
        throws BulkRequestException
    {
      ...
      {
        // This most likely indicates that the JSON represents some other
        // type.
        throw new BulkRequestException(
