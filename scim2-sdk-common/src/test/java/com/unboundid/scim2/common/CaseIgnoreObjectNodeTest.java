...
import static tools.jackson.databind.node.StringNode.valueOf;
...
public class CaseIgnoreObjectNodeTest
{
   ...
   * explicitly defined in the subclass. This ensures that these perform in a
   * case-insensitive way even without an explicit override.
   */
  @Test
  public void testBasic()
  {
    ...
    assertThat(node.findValue("DIRTY").asString()).isEqualTo("Rotten");
    assertThat(node.findValue("colors").asString()).isEqualTo("flat");

    // Test findValues().
    assertThat(node.findValues("he")).containsOnly(valueOf("Away"));
    assertThat(node.findValues("NOT")).containsOnly(valueOf("Here"));
    assertThat(node.findValues("maggots", List.of(valueOf("for"))))
        .containsExactly(valueOf("for"), valueOf("brains"));

    // Test findValuesAsString().
    assertThat(node.findValuesAsString("dirty")).containsOnly("Rotten");
    assertThat(node.findValuesAsString("COLORS")).containsOnly("flat");
    assertThat(node.findValuesAsString("baby", List.of("goes")))
