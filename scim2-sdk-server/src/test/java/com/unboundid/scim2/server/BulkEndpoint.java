...
public class BulkEndpoint
{
  ...
  /**
   * Returns a bulk response with an invalid JSON to simulate a service provider
   * returning bad data.
   *
   * @return  An invalid bulk response.
   */
  @POST
  @Path("parseError")
  @Produces({MEDIA_TYPE_SCIM, MediaType.APPLICATION_JSON})
  public Response parseError()
  {
    // Operations within a bulk response (a BulkOperationResult) must have a
    // 'method' field to be valid. Return a response without this field.
    return Response.status(Response.Status.OK)
        .type(MEDIA_TYPE_SCIM)
        .entity("""
            {
